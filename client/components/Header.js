  ...
  const profileLinks = (
    <>
      <div className="hidden md:inline-flex space-x-6 text-gray-500">
        ...
        <Link
          href="/"
          className="flex space-x-2 items-center cursor-pointer py-2 px-2  rounded-full border border-gray-500 hover:bg-blue-200"
        >
  ...
  return (
    <header className="font-mono top-0 z-50 grid grid-cols-2 font-forum  py-5 px-5 md:px-10">
      <div className="flex space-x-5">
        <Link
          href="/"
          className="relative flex items-center h-5 cursor-pointer my-auto bg-white px-2 py-5 rounded-full shadow-xl"
        >
          <HomeIcon className="h-6 text-gray-500" />
        </Link>
        {!loading && (
      ...
      <div className="hidden md:flex items-cener justify-end  whitespace-nowrap text-xl">
        {!loading && <Fragment>{profileLinks}</Fragment>}
      </div>
      <div className="flex items-cener justify-end  whitespace-nowrap text-xl">
        {!loading && (
          <Fragment>{isAuthenticated ? dropdownAuth : dropdownGuest}</Fragment>
        )}
      </div>
    </header>
  );
